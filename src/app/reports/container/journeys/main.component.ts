...
export class JourneyReportComponent implements OnInit {
  ...
  fetchRecords() {
    if (!this.employee_no) {
      this.toastService.showToastNotification('error', 'Employee Number Required', 'Error');
      return;
    }
    this.loadingService.showloading();
    const params = {
    ...
    this.administrationService.getrecords(journey_report_url, params).subscribe((res) => {
      this.records = res;
      this.loadingService.hideloading();
