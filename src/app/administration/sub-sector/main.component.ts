...
export class SubSectorComponent implements OnInit {
  ...
  editRecord(index:any) {
    const sub_sector = this.records[index]
    this.editRecordForm.patchValue(sub_sector);
    this.editRecordForm.patchValue({'sector': sub_sector?.sector?.id});
    this.editModal.show();
  }
