...
export class WardComponent implements OnInit {
  ...
  constructor(public administrationService: AdministrationService,
    ...
    this.createRecordForm = this.formBuilder.group({
      ...
      sub_county: new FormControl('', Validators.compose([Validators.required, Validators.minLength(2), ])),
    });
  ...
  fetchSubCounty() {
    ...
    this.administrationService.getrecords(sub_counties_url, params).subscribe((res) => {
      this.sub_counties = res;
      // this.loadingService.hideloading();
      ...
      'Do you wish to proceed updating record?').then((res) => {
        if (res) {
          const payload = {
            ...
            'sub_county': this.editRecordForm.get('sub_county')!.value,
          };
          this.administrationService.updaterecord(wards_url, payload).subscribe((data) => {
            if (data) {
              this.fetchRecords();
